...
export default class PostReportsController {
  ...
  async create(ctx: HttpContext) {
    ...
    try {
      ...
      const resource = this.service.create(currentUserId, post, payload)
  ...
  async update(ctx: HttpContext) {
    const postReportId = ctx.request.qs().id;
    const postReport = await PostReport.findByOrFail(postReportId);
    ...
    try {
      const payload = ctx.request.body()
      const resource = this.service.update(postReport, payload)
  ...
  async destroy(ctx: HttpContext) {
    const postReportId = ctx.request.qs().id;
    const resource = await PostReport.findByOrFail(postReportId);
